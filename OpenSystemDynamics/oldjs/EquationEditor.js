...
function EquationWindow(config) {
	...
	for (var i = 0; i < hood.length; i++) {
		if (!hood[i].linkHidden) {
			...
			neighbors.push({
				insert: "[asd" + hood[i].item.getAttribute("name") + "]",
				display: s,
				group: " Referencesaa",
				tip: undefined
			});
	...
	function addButton(title, vals) {
		...
		expandCount++;
		availableLinks.add({
			display: " <span class='moreExpander' data-id='" + expandCount + "' style='padding: 0px 8px; float:right; cursor: pointer' class='red'><i class='fa fa-question-circle' style='color: gray'></i></span>" + vals[0] + "<p id='expander" + expandCount + "' style='display:none; font-size: small; white-space: normal' class='gray'>" + tip + "</p>",
