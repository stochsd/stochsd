  ...
  var helpData = [
    [
      ...
      [
        ...
        [
          "Mean",
          "Mean(##Values$$)",
          "Returns the mean of a vector or list of numbers.",
          ["Mean(2, 7, 3)", "4"]
        ],
        [
          "Median",
    ...
    [
      ...
      [
        ...
        [
          "Time Length",
          "TL()",
          "The total length of the simulation excluding units."
        ],
        [
          "Time End",
          "TE()",
          "The time at which the simulation ends excluding units."
        ]
    ...
    [
      ...
      [
        ...
        [
          "Delay1",
          "Delay1(##[Primitive]$$, ##Delay Length$$, ##Initial Value$$)",
          "Returns a smoothed, first-order exponential delay of the value of a primitive. The Initial Value is optional.",
          "Delay1([Income], 5, 10000)"
        ],
        [
          "Delay3",
          "Delay3(##[Primitive]$$, ##Delay Length$$, ##Initial Value$$)",
          "Returns a smoothed, third-order exponential delay of the value of a primitive. The Initial Value is optional.",
          "Delay3([Income], {20 Months}, 10000)"
        ],
        [
          "Smooth",
          "Smooth(##[Primitive]$$, ##Length$$, ##Initial Value$$)",
          "Returns a smoothing of a primitive's past values. Results in an averaged curve fit. Length affects the weight of past values. The Initial Value is optional."
        ],
        [
          "PastValues",
          ...
          ["Sum(PastValues([Income]))", "Total past income"]
        ],
        [
          "Maximum",
        ...
        [
          ...
          [
            "PastMin([Income], 10)",
            "The minimum income in the past 10 units of time"
          ]
        ],
        [
          "Median",
        ...
        [
          "Fix",
          "Fix(##Value$$, ##Period=-1$$)",
          "Takes the dynamic value and forces it to be fixed over the course of the period. If period is -1, the value is held constant over the course of the whole simulation.",
          [
            "Fix(Rand(), {5 Years})",
    ...
    [
      ...
      [
        [
          "Poisson Flow",
          "PoFlow(##Lambda$$)",
          "Short for RandPoisson(DT()*Lambda)/DT()."
        ],
        [
        ...
        [
          ...
          "Returns 1 with the specified probability, otherwise 0. The probability is optional and defaults to 0.5: a coin flip.",
          ["RandBoolean(0.1)", "False"]
        ],
        [
    ...
    [
      ...
      [
        [
          "Unitless",
          "Unitless(##Value$$)",
          "Returns a unitless version of its input.",
          "Unitless(TimeStep())"
        ],
        [
          "If Then Else",
        ...
        [
          "PulseFcn",
          "PulseFcn(##Time$$, ##Volume$$, ##Repeat)",
          "Creates a pulse input at the specified time with the specified Volume. Repeat is optional and will create a pulse train with the specified time if positive..",
          "Pulse(0, 5, 2)"
        ],
        [
          "Step",
        ...
        [
          ...
          "Ramp({10 Year}, {20 Years}, 5)"
        ],
        [
          "Pause",
          "Pause()",
          "Pauses the simulation and allows sliders to be adjusted. Often used in combination with an IfThenElse function.",
          "IfThenElse(Years() = 20, Pause(), 0)"
        ],
        [
          "Stop",
          "Stop()",
          "Immediately terminates the simulation. Often used in combination with an IfThenElse function.",
...
function sdsLoadFunctions() {
  ...
  defineFunction(
    "PoFlow",
    { params: [{ name: "Rate", noUnits: true, noVector: true }] },
    function(x) {
  ...
  defineFunction(
    ...
    {
      params: [
        { name: "Start Time", vectorize: true },
        { name: "Volume", vectorize: true, defaultVal: 1 },
        { name: "Width", vectorize: true, defaultVal: 0 },
        { name: "Repeat Period", vectorize: true, defaultVal: 0 }
      ]
    },
    function(x) {
      ...
      if (repeat.value <= 0) {
        if (
          eq(simulate.time(), start) ||
          (greaterThanEq(simulate.time(), start) &&
            lessThanEq(simulate.time(), plus(start, width)))
        ) {
          return height;
        }
        ...
        var x = minus(
          simulate.time(),
          mult(
            functionBank["floor"]([div(minus(simulate.time(), start), repeat)]),
            repeat
          )
        );
        var dv = minus(simulate.time(), start);
        if (
          minus(functionBank["round"]([div(dv, repeat)]), div(dv, repeat))
            .value == 0 ||
          (greaterThanEq(x, start) && lessThanEq(x, plus(start, width)))
        ) {
          return height;
        }
      }
...
function getVisibleNeighborhoodIds(id) {
  ...
  let visibleNeighbors = neighbors.filter(neighbor => {
    return !neighbor.linkHidden;
  });
  return visibleNeighbors.map(neighbor => {
    return neighbor.item.getAttribute("id");
  });
}
...
class BaseObject {
  ...
  getMountPos(closeToPoint) {
    ...
    let closedFoundDistance = pointDistance(
      positionSum(this.mountPoints[0], pos),
      closeToPoint
    );
...
class StockVisual extends BasePrimitive {
  ...
  getImage() {
    return [
      ...
      svg_group(
        [svg_from_string(ghost_image)],
        svg_transform_string(0, 0, 0, 1),
        "ghost"
      ),
      svg_rect(-20, -15, 40, 30, "red", "none", "selector"),
...
class ConverterVisual extends BasePrimitive {
  ...
  getImage() {
    return [
      svg_path(
        "M-20 0  L-10 -15  L10 -15  L20 0  L10 15  L-10 15  Z",
        "black",
        "white",
        "element"
      ),
...
class TwoPointer extends BaseObject {
  ...
  create_dummy_start_anchor() {
    this.start_anchor = new AnchorPoint(
  ...
  create_dummy_end_anchor() {
    this.end_anchor = new AnchorPoint(
      this.id + ".end_anchor",
      "dummy_anchor",
      [this.endx, this.endy],
      anchorTypeEnum.end
...
class FlowVisual extends BaseConnection {
  ...
  updateLength() {
    ...
    var newPath = `M0,0 ${headHalfWidth},10 ${halfWidth},10 ${halfWidth},${
      this.length
    } -${halfWidth},${this.length} -${halfWidth},10 -${headHalfWidth},10 Z`;
    this.arrowPath.setAttribute("d", newPath);
  }

  makeGraphics() {
    this.arrowPath = svg_from_string(
      `<path d="M0,0 0,0" stroke="black" fill="white"/>`
    );
    this.updateLength();
...
class RiverVisual extends BaseConnection {
  ...
  createAnchorPoint(x, y) {
    let newAnchor = new AnchorPoint(
      this.id + ".point" + this.anchorPoints.length,
      "dummy_anchor",
      [x, y],
      anchorTypeEnum.bezier1
    );
...
class RectangleVisual extends TwoPointer {
  makeGraphics() {
    ...
    this.element = svg_rect(
      ...
      "element",
      dash
    ); //svg_line(this.startx,this.starty,this.endx,this.endy,"black","white","element",dash);
...
class TableVisual extends htmlTwoPointer {
  ...
  render() {
    ...
    for (let row_index in results) {
      ...
      for (let column_index in ["Time"].concat(namesToDisplay)) {
        ...
        html +=
          "<td>" + stocsd_format(results[row_index][column_index], 6) + "</td>";
      }
      html += "</tr>";
    }
    html += "</tbody></table>";
  ...
  makeGraphics() {
    ...
    this.htmlElement = svg_foreignobject(
      this.startx,
      this.starty,
      200,
      200,
      "table not renderd yet"
    );
    ...
    var element_array = [this.element];
    var element_array = [this.htmlElement.scrollDiv, this.element];
    for (var key in element_array) {
      element_array[key].setAttribute("node_id", this.id);
    }
  }
...
class htmlOverlayTwoPointer extends TwoPointer {
  ...
  makeGraphics() {
    ...
    document
      .getElementById("svgplanebackground")
  ...
  updateGraphics() {
    ...
    this.targetElement.style.left =
      this.getMinX() + this.targetBorder + 1 + "px";
    this.targetElement.style.top =
      this.getMinY() + this.targetBorder + 1 + "px";

    this.targetElement.style.width =
      this.getWidth() - 2 * this.targetBorder + "px";
    this.targetElement.style.height =
      this.getHeight() - 2 * this.targetBorder + "px";
  }
...
class DiagramVisual extends htmlOverlayTwoPointer {
  ...
  makeGraphics() {
    ...
    let html = `<div id="${
      this.chartId
    }" style="width:0px; height:0px; z-index: 100;"></div>`;
    this.updateHTML(html);
...
class LinkVisual extends BaseConnection {
  ...
  makeGraphics() {
    ...
    this.arrowPath = svg_from_string(
      `<path d="M0,0 -${headHalfWidth},7 ${headHalfWidth},7 Z" stroke="black" fill="black"/>`
    );
    this.arrowhead = svg_group([this.arrowPath]);
    svg_translate(this.arrowhead, this.endx, this.endy);
    this.click_area = svg_curve(
      this.startx,
      this.starty,
      this.startx,
      this.starty,
      ...
      this.startx,
      this.starty,
      { "pointer-events": "all", stroke: "none", "stroke-width": "10" }
    );
    this.curve = svg_curve(
      this.startx,
      this.starty,
      this.startx,
      this.starty,
    ...
    this.b1_anchor = new AnchorPoint(
      this.id + ".b1_anchor",
      "dummy_anchor",
      [this.startx, this.starty],
      anchorTypeEnum.bezier1
    ...
    this.b2_anchor = new AnchorPoint(
      this.id + ".b2_anchor",
      "dummy_anchor",
      [this.startx, this.starty],
      anchorTypeEnum.bezier2
    );

    this.b1_line = svg_line(
      this.startx,
      this.starty,
      this.startx,
      this.starty,
    ...
    this.element_array = this.element_array.concat([
      this.b1_line,
      this.b2_line
    ]);
  }
...
class ConverterTool extends BaseTool {
  static leftMouseDown(x, y) {
    ...
    var new_converter = createPrimitive(
      primitive_name,
      "Converter",
      [x - size[0] / 2, y - size[1] / 2],
      size
...
class TwoPointerTool extends BaseTool {
  ...
  static leftMouseDown(x, y) {
    ...
    this.primitive.subscribePosition(
      this.current_connection.positionUpdateHandler
    );
...
class FlowTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new FlowVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class RiverTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new RiverVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class LinkTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new LinkVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class RectangleTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new RectangleVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class LineTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new LineVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class TableTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new TableVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class DiagramTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new DiagramVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class TextAreaTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new TextAreaVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
class XyPlotTool extends TwoPointerTool {
  static create_TwoPointer_start(x, y, name) {
    ...
    this.current_connection = new XyPlotVisual(
      this.primitive.id,
      this.get_type(),
      [x, y]
    );
...
function attach_selected_anchor(selectedAnchor) {
  ...
  for (var i = 0; i < elements_under.length; i++) {
    if (
      !elements_under[i].is_selected() &&
      "getMountPos" in elements_under[i] &&
      elements_under[i] != parentConnection
    ) {
...
function in_selection(node_id) {
  if (
    ...
    object_array[node_id].pos[0] <=
      rectselector.xmin() + rectselector.width() &&
...
class Clipboard {
  ...
  static copyObject(clipboardItem) {
    ...
    setCenterPosition(vertex, [
      lastMouseX + relativePosition[0],
      lastMouseY + relativePosition[1]
    ]);
    ...
    for (let i in this.copiedItems) {
      do_global_log(
        "hoj " +
          JSON.stringify(
            positionDifference(
              this.copiedItems[i].absolutePosition,
              centerPosition
            )
          )
      );
      this.copiedItems[i].relativePosition = positionDifference(
        this.copiedItems[i].absolutePosition,
        centerPosition
      );
    }
...
function find_connections(primitive) {
  return find_start_connections(primitive).concat(
    find_end_connections(primitive)
  );
}
...
function syncVisual(tprimitive) {
  ...
  switch (nodeType) {
    case "Numberbox":
      {
        ...
        let visualObject = new NumberboxVisual(
          tprimitive.id,
          "numberbox",
          position
        );
    ...
    case "Converter":
      {
        var position = getCenterPosition(tprimitive);
        let visualObject = new ConverterVisual(
          tprimitive.id,
          "converter",
          position
        );
    ...
    case "Ghost":
      {
        ...
        switch (source_type) {
          case "Converter":
            visualObject = new ConverterVisual(
              tprimitive.id,
              "converter",
              position,
              { is_ghost: true }
            ...
            break;
          case "Variable":
            visualObject = new VariableVisual(
              tprimitive.id,
              "variable",
              position,
              { is_ghost: true }
            ...
            break;
          case "Stock":
            visualObject = new StockVisual(tprimitive.id, "stock", position, {
              is_ghost: true
            });
            break;
        }
    ...
    case "Link":
      {
        ...
        if (tprimitive.source != null) {
          // Attach to object
          connection.start_attach = get_object(
            tprimitive.source.getAttribute("id")
          );
        } else {
          // Set UI-coordinates to coordinates in primitive
          connection.start_anchor.set_pos(source_position);
        ...
        if (tprimitive.target != null) {
          // Attach to object
          connection.end_attach = get_object(
            tprimitive.target.getAttribute("id")
          );
        } else {
          // Set UI-coordinates to coordinates in primitive
          connection.end_anchor.set_pos(target_position);
          ...
          case "Link":
            {
              ...
              if (bezierPoints.indexOf(null) == -1) {
                connection.b1_anchor.set_pos([
                  Number(bezierPoints[0]),
                  Number(bezierPoints[1])
                ]);
                connection.b2_anchor.set_pos([
                  Number(bezierPoints[2]),
                  Number(bezierPoints[3])
                ]);
              } else {
  ...
  for (let type of saveblePrimitiveTypes) {
    ...
    for (key in primitive_list) {
      try {
        ...
        alert(
          "Error while loading corrupted primitive of type " +
            type +
            ". Removing corrupted primitive to avoid propagated errors."
        );
  ...
  static init() {
    $(document).ready(() => {
      $("#svgBlockOverlay").mousedown(() => {
        ...
        yesNoAlert(
          "Do you want to terminate the simulation now to change the model?",
          function(answer) {
...
class RunResults {
  ...
  static updateProgressBar() {
    ...
    $("#runStatusBarText").html(
      this.getRunProgress() + " / " + this.getRunProgressMax()
    );
  }
  ...
  static getSelectiveIdResults(varIdList) {
    ...
    do_global_log(
      "this.varIdList " +
        JSON.stringify(this.varIdList) +
        " varIdList " +
        JSON.stringify(varIdList)
    );
    let returnResults = [];
    for (let row_index in this.results) {
      let tmpRow = [];
...
function getPrimitiveList() {
  let primitiveList = primitives("Stock")
    .concat(primitives("Flow"))
    .concat(primitives("Variable"))
    .concat(primitives("Converter"));
    ...
    return `
        ...
        .map(
          p =>
            ...
            '</td><td><input class="primitive_checkbox" type="checkbox" ' +
...
class DiagramDialog extends DisplayDialog {
      ...
      `
		...
		<table>
			...
			<tr>
				...
				<td><input class="xAuto intervalsettings" type="checkbox" ${checkedHtmlAttribute(
          this.xAuto
        )}></td>
			</tr>
			<tr>
				...
				<td><input class="yAuto intervalsettings" type="checkbox" ${checkedHtmlAttribute(
          this.yAuto
        )}></td>
			</tr>
		</table>
  ...
  updateInterval() {
    ...
    this.xAuto = $(this.dialogContent)
      .find(".xAuto")
      .prop("checked");

    $(this.dialogContent)
      .find(".xMin")
      .prop("disabled", this.xAuto);
    $(this.dialogContent)
      .find(".xMax")
      .prop("disabled", this.xAuto);
    ...
    this.yAuto = $(this.dialogContent)
      .find(".yAuto")
      .prop("checked");

    $(this.dialogContent)
      .find(".yMin")
      .prop("disabled", this.yAuto);
    $(this.dialogContent)
      .find(".yMax")
      .prop("disabled", this.yAuto);
...
class TableDialog extends DisplayDialog {
    ...
    let contentHTML =
      ...
      `
		...
		<tr>
			...
			<td><input class="intervalsettings start" name="start" value="${
        this.start
      }" type="text"></td>
			<td>Auto <input class="intervalsettings start_auto" type="checkbox"  ${checkedHtmlAttribute(
        this.startAuto
      )}/></td>
		</tr><tr>
			<td class="text">To</td>

			<td><input class="intervalsettings end" name="end" value="${
        this.end
      }" type="text"></td>
			<td>Auto <input class="intervalsettings end_auto" type="checkbox"  ${checkedHtmlAttribute(
        this.endAuto
      )}/></td>
		</tr><tr>
			<td class="text">Step</td>
			<td><input class="intervalsettings step" name="step" value="${
        this.step
      }" type="text"></td>
			<td>Auto <input class="intervalsettings step_auto" type="checkbox"  ${checkedHtmlAttribute(
        this.stepAuto
      )}/></td>
		</tr>
		</table>
  ...
  updateInterval() {
    ...
    this.startAuto = $(this.dialogContent)
      .find(".start_auto")
      .prop("checked");
    $(this.dialogContent)
      .find(".start")
      .prop("disabled", this.startAuto);
    $(this.dialogContent)
      .find(".start")
      .val(this.getStart());

    this.endAuto = $(this.dialogContent)
      .find(".end_auto")
      .prop("checked");
    $(this.dialogContent)
      .find(".end")
      .prop("disabled", this.endAuto);
    $(this.dialogContent)
      .find(".end")
      .val(this.getEnd());

    this.stepAuto = $(this.dialogContent)
      .find(".step_auto")
      .prop("checked");
    $(this.dialogContent)
      .find(".step")
      .prop("disabled", this.stepAuto);
    $(this.dialogContent)
      .find(".step")
      .val(this.getStep());
  }
...
class SimulationSettings extends jqDialog {
  ...
  afterOkClose() {
    let timeStart = $(this.dialogContent)
      .find(".input_start")
      .val();
    setTimeStart(timeStart);

    let timeLength = $(this.dialogContent)
      .find(".input_length")
      .val();
    setTimeLength(timeLength);

    let timeStep = $(this.dialogContent)
      .find(".input_step")
      .val();
...
class ConverterDialog extends jqDialog {
  constructor() {
    ...
    this.setHtml(`
		...
		`);
    this.valueField = $(this.dialogContent)
      .find(".valueField")
      .get(0);
    this.nameField = $(this.dialogContent)
      .find(".nameField")
      .get(0);
  }
  open(id, defaultFocusSelector = null) {
    ...
    if (this.defaultFocusSelector) {
      let valueFieldDom = $(this.dialogContent)
        .find(this.defaultFocusSelector)
        .get(0);
      valueFieldDom.focus();
    }
  }
...
function do_global_log(line) {
  global_log =
    line + "; " + new Date().getMilliseconds() + "<br/>" + global_log;
  global_log_update();
}
...
class EquationEditor extends jqDialog {
  ...
  <div class="table-row">
    ...
    this.nameField = $(this.dialogContent)
      .find(".nameField")
      .get(0);
    this.referenceDiv = $(this.dialogContent)
      .find(".referenceDiv")
      .get(0);
    this.restrictPositiveCheckbox = $(this.dialogContent)
      .find(".restrictPositive")
      .get(0);
    this.positiveOnlyDiv = $(this.dialogContent)
      .find(".positiveOnlyDiv")
      .get(0);
    ...
    $(this.dialogContent)
      .find(".clickFunction")
      .click(event => this.templateClick(event));
  ...
  open(id, defaultFocusSelector = null) {
    ...
    if (referenceList.length > 0) {
      referenceHTML =
        "Linked primitives: <br/>" + referenceListToHtml(referenceList);
    } else {
  ...
  templateClick(event) {
    ...
    let newValue =
      oldValue.slice(0, this.valueSelectionStart) +
      templateData +
      oldValue.slice(this.valueSelectionEnd);
  ...
  restoreValueSelectionRange() {
    ...
    this.valueField.setSelectionRange(
      this.valueField.selectionStart,
      this.valueField.selectionEnd
    );
  }
  afterOkClose() {
    if (this.primitive) {
      // Handle value
      let value = $(this.dialogContent)
        .find(".valueField")
        .val();
      value = value.replace(/\n/g, "\\n");
      setValue(this.primitive, value);
...
class MacroDialog extends jqDialog {
  ...
  afterOkClose() {
    let newMacro = $(this.dialogContent)
      .find(".macroText")
      .val();
    setMacros(newMacro);
  }
...
class TextAreaDialog extends jqDialog {
  ...
  afterOkClose() {
    let newText = $(this.dialogContent)
      .find(".text")
      .val();
