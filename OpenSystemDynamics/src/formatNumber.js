 ...
 */
function roundExponent(exponent) {
    return Math.floor(exponent / 3) * 3;
}
class Digits {
    constructor(integerDigits, decimalDigits) {
        this.digits = {};
    ...
    round(desiredRoundingPos) {
        if (desiredRoundingPos < this.bottomPosition)
            return this;
    ...
    toString() {
        let result = "";
        this.iterate(this.topPosition, this.bottomPosition, (position) => {
            if (position == -1) {
                result += ".";
            }
...
/*
    ...
    digits:     8 3 4 2 5 . 4 1 0 9 8 4
                            above       below
*/
function roundDigits(integerDigits, decimalDigits, position) {
...
export function formatNumber(value, options = {}) {
    ...
    for (let i = selectedExp; i > selectedExp - 10; i--) {
        let tmpNum = Math.floor(rest / (Math.pow(10, i)));
        if (i === selectedExp) {
...
export function decimalsInValueString(value) {
...
window.decimalsInValueString = decimalsInValueString;
