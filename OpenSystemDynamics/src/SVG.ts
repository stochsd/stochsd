...
export namespace SVG {
	/* replaces svgplane */
	export let svgElement: SVGElement = document.getElementById("svgplane") as unknown as SVGElement;
	...
	export function widePath(width: number, color: string, extraAttributes?: Record<string, string>) {
	...
	export function cloud(stroke: string, fill: string, extraAttributes: Record<string, string>) {
		...
		result.setPosition = function (pos: Point, adjacentPos: Point) {
			...
			this.pos = (window as any).translate(pos, offset);
		}
